...
```text
index.html / soundtracks.html / album.html / top-10.html / top-20.html
...
- **The HTML entry files are part of the runtime contract**: `index.html`, `soundtracks.html`, `album.html`, `top-10.html`, and `top-20.html` are referenced by `vite.config.ts`, shared navigation in `src/site.ts`, renderer links in `src/index.ts` and `src/album.ts`, Playwright tests, and the scaffolder output. Moving or renaming them is a route change, not a cosmetic cleanup.
