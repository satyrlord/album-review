...
(function () {
    ...
    function renderHeroMedia(coverUrl, artist, title) {
        if (!coverUrl)
            return '';
    ...
    function renderPage(d) {
        ...
        if (d.label)
            metaRows.push(`      <div><strong>Label:</strong> ${esc(d.label)}</div>`);
        if (d.producer)
            metaRows.push(`      <div><strong>Producer:</strong> ${esc(d.producer)}</div>`);
        if (d.genre)
            metaRows.push(`      <div><strong>Genre:</strong> ${esc(d.genre)}</div>`);
    ...
    async function main() {
        ...
        if (!id) {
            showError('No album ID specified in URL (?id=…).');
            return;
        }
